...
impl AgentCheckpointPreset for CodexPreset {
    fn run(&self, flags: AgentCheckpointFlags) -> Result<AgentRunResult, GitAiError> {
        ...
        // For PreToolUse with file-edit tools, return a Human checkpoint immediately
        // without parsing the transcript — the edit hasn't happened yet.
...
mod codex_tests {
    ...
    fn test_codex_pretooluse_returns_human_checkpoint() {
        ...
        assert!(matches!(run.checkpoint_kind, CheckpointKind::Human));
        assert_eq!(run.agent_id.tool, "codex");
        assert_eq!(run.agent_id.model, "gpt-5.4-mini");
        assert_eq!(run.agent_id.id, "019d4dfb-940e-71d1-bc4c-951d0e38b8dd");
        assert_eq!(run.repo_working_dir.as_deref(), Some("/tmp/test-repo"));
        assert!(run.transcript.is_none());
        assert!(run.edited_filepaths.is_none());
        let will_edit = run
            .will_edit_filepaths
            .expect("should have will_edit_filepaths");
    ...
    fn test_codex_posttooluse_returns_ai_agent_checkpoint() {
        let input = serde_json::json!({
            ...
            "model": "gpt-5.4-mini",
            "tool_name": "apply_patch",
            "tool_input": {"files": ["poems/test.txt", "src/main.rs"]},
            "tool_use_id": "call_abc123"
        });
        ...
        let run = result.unwrap();
        assert!(matches!(run.checkpoint_kind, CheckpointKind::AiAgent));
        assert_eq!(run.agent_id.model, "gpt-5.4-mini");
        assert!(run.transcript.is_some());
        let edited = run.edited_filepaths.expect("should extract edited files");
        assert_eq!(edited, vec!["poems/test.txt", "src/main.rs"]);
    }
    ...
    fn test_codex_posttooluse_falls_back_to_tool_response() {
        ...
        let edited = run
            .edited_filepaths
            .expect("should fall back to tool_response");
    ...
    #[test]
    fn test_extract_filepaths_from_tool_input_empty_files() {
        let hook_data = serde_json::json!({
            "tool_input": {"files": []}
        });
        assert!(CodexPreset::extract_filepaths_from_tool_input(&hook_data).is_none());
    ...
    #[test]
    fn test_extract_filepaths_from_tool_response_no_matching_lines() {
        let hook_data = serde_json::json!({
            "tool_response": {"output": "No changes made.\n"}
        });
        assert!(CodexPreset::extract_filepaths_from_tool_response(&hook_data).is_none());
    }
}
